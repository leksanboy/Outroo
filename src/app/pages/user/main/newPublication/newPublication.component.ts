...
export class MainNewPublicationComponent implements OnInit {
	...
	constructor(
		...
		public dialogRef: MatDialogRef<MainNewPublicationComponent>,
	...
	openPhotos(event: Event){
		...
		let dialogRef = this.dialog.open( MainNewPublicationAddPhotosComponent, config);
	...
	openAudios(event: Event){
		...
		let dialogRef = this.dialog.open( MainNewPublicationAddAudiosComponent, config);
		dialogRef.afterClosed().subscribe((res: any) => {
			this.location.go('/' + this.sessionData.current.username + '#newPublication');
